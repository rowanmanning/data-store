...
const MultipleValidationError = require('./multiple-validation-error');
...
class DataStore {
	 ...
	 *     Throws a validation error if any of the data values are invalid.
	 */
	constructor(data = {}) {
	 ...
	 *     Throws if the property name is not valid.
	 * @throws {ValidationError}
	 *     Throws a validation error if `property` is not an allowed property.
	 * @throws {ValidationError}
	 *     Throws a validation error if `value` is invalid.
	 */
	set(property, value) {
	...
	_setOne(property, value) {
		...
		if (!this.constructor.isAllowedProperty(normalizedProperty)) {
			...
			this.invalidate(
				`${classAndProperty} is not an allowed property name`,
				{},
				'DISALLOWED_PROPERTY'
			);
		}
	 ...
	 * @throws {TypeError}
	 *     Throws if properties is not an object, or if any of the property names are not valid.
	 * @throws {ValidationError}
	 *     Throws a validation error if any of the properties are not allowed.
	 * @throws {ValidationError}
	 *     Throws a validation error if any of the property values are invalid.
	 */
	_setMany(properties) {
	 ...
	 */
	invalidate(message, details, code) {
...
/**
 * A reference to the {@link MultipleValidationError} class.
 *
 * @access public
 * @type {Function}
 */
DataStore.MultipleValidationError = MultipleValidationError;
